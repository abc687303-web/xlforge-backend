...
# DifferentialStyle removed - use CellIsRule with fill/font directly
...
4. CHARTS: type=bar/line/pie/area; data_cols and category_col are 0-indexed integers.
   - If user says "chart", "graph", "plot", "visualize" -> ALWAYS include chart. Never skip it.
   - "chart in 2nd sheet" -> add "sheet":1 inside chart object.
   - When in doubt if data is comparative or trend-based -> add a chart.
...
9. FILE READING: when a file is uploaded, read and process EVERY row. Never ignore or skip rows. Solve math problems, fill answer columns, complete missing data.
...
def solve_math_expression(expr: str):
    """
    Safely evaluate a simple arithmetic expression like '20 + 14' or '150 + 104'.
    Only allows digits, spaces, and + - * / ( ) . operators.
    Returns int/float result, or None if it cannot be parsed.
    """
    ...
    if not re.match(r'^[\d\s\+\-\*\/\(\)\.]+$', cleaned):
        return None
    # Must actually contain an operator (otherwise it's just a plain number)
    if not re.search(r'[\+\-\*\/]', cleaned):
        return None
    try:
        result = eval(cleaned, {"__builtins__": {}}, {})  # safe: no builtins, no names
...
def sanitize_ai_response(data: dict) -> dict:
    for sheet in data.get("sheets", []):
        ...
        # Identify answer columns - these should receive computed results
        answer_col_indices = set()
        for i, h in enumerate(headers):
            hl = str(h).lower()
            if any(k in hl for k in ["answer", "result", "solution", "output", "total"]):
                answer_col_indices.add(i)
        ...
        for row in sheet.get("rows", []):
            if isinstance(row, list):
                ...
                # Build new row with problem-column protection and answer-column computation
                new_row = []
                for ci, v in enumerate(row):
                    if ci in problem_col_indices:
                        # Keep problem text as string - never coerce math expressions
                        new_row.append(str(v) if v is not None else "")
                    elif ci in answer_col_indices:
                        # If the cell is empty, "#VALUE!", or a non-numeric string,
                        # try to solve the math from the nearest problem column in this row
                        str_v = str(v).strip() if v is not None else ""
                        is_bad = (
                            str_v == ""
                            or str_v.upper().startswith("#")  # catches #VALUE!, #REF!, etc.
                            or (not isinstance(v, (int, float)) and not str_v.lstrip("-").replace(".", "", 1).isdigit())
                        )
                        if is_bad:
                            # Look for a problem column in this row to solve from
                            solved = None
                            for pi in problem_col_indices:
                                if pi < len(row):
...
- Use every single row from the file above - do not skip or reduce rows
- Complete any missing values, answers, or calculations in the file
- If task says "add chart" or "make chart" -> you MUST include chart in JSON
- If task says "add column" -> add that column to the data
...
- Return only JSON"""
    ...
    # Content limits - generous on first attempt to capture all rows
    CONTENT_LIMITS = [8000, 5000, 3000, 1500]
...
def build_excel(data: dict, output_path: str, password: str = None):
    ...
    for pc in pending_charts:
        try:
            ...
            if ctype == "pie":
                dc = dcols[0]
                max_col = data_ws.max_column or 99
                if dc + 1 > max_col:
                    dc = 1
